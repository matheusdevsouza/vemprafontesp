...
export function MizunoProphecy8Section() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true)
        const response = await fetch('/api/products?subcategory_id=5')
        ...
        console.error('Erro ao buscar produtos Mizuno Prophecy 8:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) {
    return (
      <section ref={sectionRef} className="py-24 bg-dark-900 overflow-hidden">
        <div className="container mx-auto px-4">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Prophecy 8</h2>
            <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto">
              A mais recente evolução da linha Prophecy com tecnologia inovadora
            </p>
          </motion.div>
