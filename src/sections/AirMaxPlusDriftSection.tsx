...
export function AirMaxPlusTNSection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true)
        const response = await fetch('/api/products?subcategory_id=8')
        ...
        console.error('Erro ao buscar produtos Air Max Plus TN:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <section ref={sectionRef} className="py-24 bg-dark-950 overflow-hidden">
        <div className="container mx-auto px-4">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Air Max Plus TN</h2>
          ...
          <div className="max-w-[1400px] mx-auto overflow-hidden">
            <div className="flex gap-6 pb-8 justify-center">
              {[...Array(5)].map((_, index) => (
                <AirMaxPlusTNSkeleton key={index} />
              ))}
            </div>
