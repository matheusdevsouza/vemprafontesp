...
export function AirMax95Section() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true)
        const response = await fetch('/api/products?subcategory_id=1')
        ...
        console.error('Erro ao buscar produtos Air Max 95:', error)
      } finally {
        setLoading(false)
      }
    }
    ...
    return (
      <section ref={sectionRef} className="py-24 bg-dark-950 overflow-hidden">
        <div className="container mx-auto px-4">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Air Max 95</h2>
            <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto">
              Design icônico inspirado na anatomia humana
            </p>
          </motion.div>
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-dark-950 overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-[1400px] mx-auto overflow-hidden">
          {/* Botões de Navegação */}
          <div className="hidden md:block absolute top-1/2 -translate-y-1/2 -left-16">
            <button
              onClick={slideLeft}
          ...
          {Math.floor((products.length - 1) / itemsPerPage) > 0 && (
            <div className="flex justify-center gap-3 sm:gap-4 mt-6 sm:mt-8">
              ...
              >
                <CaretLeft size={18} className="sm:w-6 sm:h-6" weight="bold" />
              </button>
              ...
              >
                <CaretRight size={18} className="sm:w-6 sm:h-6" weight="bold" />
              </button>
            </div>
          )}
        </div>
